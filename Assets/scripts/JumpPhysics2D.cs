...
public class JumpPhysics2D : MonoBehaviour
{
    ...
    public List<string> collidingLayerList;

    // amount ray extends beyond sprite 
    public float collisionRayDepthUp = .1f;
    public float collisionRayDepthDown = .1f;

    // offset each ray to the left/right before casting up/down to match character
    public Vector2 collisionRayLeftOffset = Vector2.zero;
    public Vector2 collisionRayRightOffset = Vector2.zero;
    ...
    void Awake()
    {
        ...
        // if no tags or layers defined at start, use "Default" layer as collidable.
        if ((collidingTagList == null || collidingTagList.Count == 0) && (collidingLayerList == null || collidingLayerList.Count == 0)) {
            collidingLayerList = new List<string> {"Default"};
        } else if (collidingLayerList != null && collidingLayerList.Count > 0) { // if we have layer list, ensure the layers actually exist in the game.
            foreach (string layerName in collidingLayerList) {
                if ((LayerMask.NameToLayer(layerName) == -1)) {
                    Debug.LogError("[Jump Physics] Layer " + layerName + " is not a real layer.");
                }
            }
        }
    ...
    bool RaycastCollideVertical(Vector2 direction)
    {
        ...
        RaycastHit2D hitLeftSide;
        RaycastHit2D hitRightSide;
        if (playerLayerName.Length == 0) { // we haven't assigned player layer mask
        
            // cast rays for collision checking
            hitLeftSide = Physics2D.Raycast((Vector2)transform.position + collisionRayLeftOffset, direction, this.renderer.bounds.extents.y + directionDepth);
            hitRightSide = Physics2D.Raycast((Vector2)transform.position + collisionRayRightOffset, direction, this.renderer.bounds.extents.y + directionDepth);
            
        } else { // player layer mask assigned
        
            // assert we have a real layer mask
            if ((LayerMask.NameToLayer(playerLayerName) == -1)) {
                Debug.LogError("[Jump Physics] Player Layer Name is not a real layer.");
            }
            
            // cast rays for collision checking
            hitLeftSide = Physics2D.Raycast((Vector2)transform.position + collisionRayLeftOffset, direction, this.renderer.bounds.extents.y + directionDepth, LayerMask.NameToLayer(playerLayerName));
    ...
    bool HitTagOrLayerObject(GameObject collisionObject)
    {
        ...
        // check if we are colliding by layer
        int objLayer = collisionObject.layer;
        if (collidingLayerList.Count > 0) {
            foreach (string layer in collidingLayerList) {
                if (LayerMask.LayerToName(objLayer) == layer) {
                    return true;
                }
            }
        }
    ...
    // draws all rays in up and down direction (down magenta, up red)
    void DebugRays()
    {
